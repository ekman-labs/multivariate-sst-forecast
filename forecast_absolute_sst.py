"""
forecast_2025_production_absolute.py - Real-World Absolute SST Forecast

SCENARIO:
We are in Dec 2024. We want to predict the ACTUAL temperature (e.g., 29.5°C)
for 2025. We have NO ground truth data for 2025.

OUTPUT:
1. forecast_2025_absolute.csv (The predictions)
2. forecast_plot.png
...
# CONFIGURATION
# ============================================================================
SST_INDO_FILE = "data/processed/sst_indo_clean.csv" # Data sampai Dec 2024
NINO34_FILE = "data/raw/nina34.anom.data.txt"       # Data sampai Dec 2024

os.makedirs("output/models", exist_ok=True)
os.makedirs("output/figures", exist_ok=True)
os.makedirs("output/tables", exist_ok=True) # Folder untuk simpan hasil CSV

CHECKPOINT_PATH = "output/models/production_model_absolute.pt"
...
def load_historical_data(sst_file, nino_file):
    """Load historical data up to Dec 2024 (Absolute SST)."""
...
def train_model(model, train_loader, val_loader, epochs, lr):
    ...
    print("\n" + "=" * 50)
    print(f"TRAINING PRODUCTION MODEL (Absolute SST)")
    print("=" * 50)
...
def recursive_forecast(model, initial_sequence, scaler, n_months=12):
    ...
    print("\n  Generating Blind Forecast for 2025 (Absolute Temp)...")
    ...
    for month in range(n_months):
        ...
        # NOTE: Tidak ada print "Actual" karena kita tidak punya datanya
        if month < len(months):
             # Kita akan print nanti setelah di-inverse transform
             pass
...
def save_and_plot(pred_sst, pred_nino, train_losses, val_losses):
    months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
    ...
    df = pd.DataFrame({
        'Date': dates,
        'SST_Absolute_Forecast': pred_sst,
        'Nino34_Forecast': pred_nino
    })
    
    csv_path = 'output/tables/forecast_2025_absolute.csv'
    df.to_csv(csv_path, index=False)
    
    print("\n" + "=" * 50)
    print("OFFICIAL 2025 FORECAST")
    print("=" * 50)
    print(df[['Date', 'SST_Absolute_Forecast']].to_string(index=False))
    ...
    # Plot 1: SST Absolute
    ax1 = axes[0]
    ax1.plot(x, pred_sst, 'r--s', linewidth=2.5, markersize=8, label='SST Forecast (Absolute)')
    # Tambahkan label angka di grafik agar jelas
    for i, v in enumerate(pred_sst):
        ax1.text(i, v + 0.1, f"{v:.1f}°C", ha='center', fontsize=9)
        
    ax1.set_ylabel('Temperature (°C)', fontsize=12)
    ax1.set_title('Official 2025 Forecast: Indonesian Sea Surface Temperature', fontweight='bold')
    ax1.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig('output/figures/forecast_2025_absolute_official.png', dpi=150, bbox_inches='tight')
    plt.close()
    print(f"✓ Saved Plot to: output/figures/forecast_2025_absolute_official.png")
...
def main():
    print("=" * 70)
    print("PRODUCTION FORECASTING: Year 2025 (ABSOLUTE SST)")
    print("Scenario: Standing in Dec 2024. Predicting 2025 blindly.")
    print("=" * 70)
    
    # 1. Load Data
    print("\n[Step 1/5] Loading historical data (Absolute)...")
    train_df = load_historical_data(SST_INDO_FILE, NINO34_FILE)
    
    # 2. Normalize (Range 0-1 for Absolute values)
    ...
    # 90% Training, 10% Internal Validation (not test)
    ...
    # Save & Visualize
    save_and_plot(pred_sst, pred_nino, train_losses, val_losses)
